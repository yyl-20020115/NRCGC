...
public:
    static ReferenceRingBuffer* Create(intptr_t count, intptr_t idle_count = DefaultIdleCount) {
        unsigned char* address = (unsigned char*)
            malloc(sizeof(ReferenceRingBuffer) + count * sizeof(Reference) + count * sizeof(intptr_t));
        if (address != nullptr) {
...
public:
    ReferenceRingBuffer(unsigned char* address, intptr_t count, intptr_t idle_count = DefaultIdleCount)
...
public:
    intptr_t Allocate(intptr_t size) {
        ...
        if (ptr != nullptr) {
            intptr_t p = this->Send(Reference(ptr, size));
            if (p != -1LL) {
#ifdef _WIN32
#ifdef _WIN64
...
#ifdef _WIN32
#ifdef _WIN64
            InterlockedAdd64((intptr_t*)&((unsigned char*)this + this->pflags_)[handle & this->capacity_mask], 0);
#else
            InterlockedAdd((intptr_t*)&((unsigned char*)this + this->pflags_)[handle & this->capacity_mask], 0);
#endif
#else
